...
class CBController extends Controller {
	...
	public function getIndex() {
		...
		foreach($columns_table as $index => $coltab) {
			...
			$join = @$coltab['join'];
			$join_catalogo = @$coltab['join_catalogo'];
			$field = @$coltab['name'];
			...
			if($join) {
			...
			}else if($join_catalogo) {

				$join_exp     = explode(',', $join_catalogo);

				$join_table  = $join_exp[0];//app_catalogodetalle
				$join_id_catalogo = $join_exp[1];//id_app_catalogo (4)
				$join_alias  = $join_table;//app_catalogodetalle
				...
				$result->leftjoin('app_catalogodetalle as joincatalogo','joincatalogo.numvalor','=',$table.'.'.$field);
				$result->addselect('joincatalogo.desnombre as joincatalogo_desvalor')
							 ->where('joincatalogo.id_app_catalogo', '=', $join_id_catalogo);


				$alias[] = $join_alias;
				$columns_table[$index]['type_data']	 = CRUDBooster::getFieldType('joincatalogo','joincatalogo.desvalor');
				$columns_table[$index]['field']      ='joincatalogo_desvalor';
				$columns_table[$index]['field_with'] = 'joincatalogo_desvalor';
				$columns_table[$index]['field_raw']  = 'desvalor';
			}
			else{
